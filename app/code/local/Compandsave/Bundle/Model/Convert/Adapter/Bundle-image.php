...
if ($update_product->getId()){
    $mediaApi = Mage::getModel("catalog/product_attribute_media_api");
    $items = $mediaApi->items($update_product->getId());
    foreach($items as $item)
        $mediaApi->remove($update_product->getId(), $item['file']);
}
   //$update_product->save();             
                /**
							...
							foreach($mediaArray as $imageType => $fileName) {
								if($fileName != ''){
									...
									if (!file_exists($filePath)) {
										$filePath = $importDir.strtoupper($sku).".jpg";
									}

									 if ( file_exists($filePath) ) {
										try {
											$update_product->getTypeInstance(true)->setStoreFilter($store_code, $update_product);
											$update_product->addImageToMediaGallery($filePath, array("image", "small_image", "thumbnail"), false, false);
										} catch (Exception $e) {
											echo $e->getMessage();
										}
									} else {
