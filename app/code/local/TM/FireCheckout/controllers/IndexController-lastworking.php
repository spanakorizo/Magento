...
//        }
        ...
        $this->getCheckout()->applyDefaults()->initCheckout(); //code changes in firecheckout/type_standard method apply default
        $this->loadLayout();
    ...
    public function saveBillingAction()
    {
        ...
        if (!$quote->isVirtual()
			...
			//$shippingAddress->setCollectShippingRates(true)->collectShippingRates(); added from another logic
			Mage::getModel('compandsave_productselector/quotes')->collectTotal();
    ...
    public function saveCouponAction()
    {
        ...
        if ($this->getRequest()->getPost('amcoupon_code_cancel', false)) {
            ...
            foreach ($appliedCoupons as $i => $coupon) {
				...
				//---change code for coupon action
				// need to chnage code zahed
        ...
        Mage::getModel('compandsave_productselector/quotes')->collectTotal(); // coupon validation is inside collectTotals method
        $sections[] = 'review';
