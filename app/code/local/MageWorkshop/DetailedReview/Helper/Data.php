...
class MageWorkshop_DetailedReview_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function checkAvailableFilter($key){
        ...
        $this->_reviewsCountWithoutFilters = $collection->getSize();
        $collection->resetTotalRecords();
        if ( isset ($params['st']) ) {
            if ($params['st'] != 0 && $params['st'] != 999) {
                $collection->addDateRangeFilter($params['st']);
            }
        }
        if ( isset ($params['vb']) ) { $collection->addVerifiedBuyersFilter(); }
        if ( isset ($params['keywords']) ) { $collection->addKeywordsFilter($params['keywords']); }
        if ( isset ($params['vr']) ) { $collection->addVideoFilter(); }
        if ( isset ($params['ir']) ) { $collection->addImagesFilter(); }
        if ( isset ($params['mr']) ) { $collection->addManuResponseFilter(); }
        if ( isset ($params['hc']) ) { $collection->addHighestContributorFilter(); }

        switch ($key) {
            case 'vb': $collection->addVerifiedBuyersFilter(); break;
            case 'vr': $collection->addVideoFilter(); break;
            case 'ir': $collection->addImagesFilter(); break;
            case 'mr': $collection->addManuResponseFilter(); break;
            case 'hc': $collection->addHighestContributorFilter(); break;
        }
