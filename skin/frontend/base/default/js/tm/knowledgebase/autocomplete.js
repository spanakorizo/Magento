...
var KnowledgebaseAutocomplete = function(el, options){
  this.el = $(el);
  ...
  if(KnowledgebaseAutocomplete.isDomLoaded){
    this.initialize();
  }else{
...
KnowledgebaseAutocomplete.instances = [];
KnowledgebaseAutocomplete.isDomLoaded = false;

KnowledgebaseAutocomplete.getInstance = function(id){
  var instances = KnowledgebaseAutocomplete.instances;
  var i = instances.length;
  while(i--){ if(instances[i].id === id){ return instances[i]; }}
};

KnowledgebaseAutocomplete.highlight = function(value, re){
  return value.replace(re, function(match){ return '<strong>' + match + '<\/strong>' });
};

KnowledgebaseAutocomplete.prototype = {
  ...
  initialize: function() {
    ...
    this.instanceId = KnowledgebaseAutocomplete.instances.push(this) - 1;
  },
  ...
  getSuggestions: function() {
    ...
    if (cr && Object.isArray(cr.suggestions)) {
      ...
      this.suggest();
        ...
        if (!isBaseUrl && this.options.baseUrl.indexOf('index.php')) { // fix for base url with index.php/
            isBaseUrl = (0 === currentUrl.indexOf(this.options.baseUrl.replace('index.php/', '')));
        }
  ...
  suggest: function() {
    ...
    this.suggestions.each(function(value) {
        ...
        content.push(
            ...
            image,
            p,
                KnowledgebaseAutocomplete.highlight(value.name, re),
                description,
            '</p>',
            '</div>'
        );
...
Event.observe(document, 'dom:loaded', function(){ KnowledgebaseAutocomplete.isDomLoaded = true; }, false);